...
async function runCheckpoint(agent: Agent, turn: number, step: number, state: CheckpointState, config: Config, deps: CheckpointDeps, signal: AbortSignal): Promise<void> {
  ...
  const base = {
    ...
    onCall: config.verbose
      ? (info: { kind: string; criterion: string; repeat: number; source: string; durationMs: number; error?: string; detail?: string }) => deps.log.info(`dsh-verifier-gate: checkpoint ${info.kind} ${info.criterion}#${info.repeat} ${info.source} ${info.durationMs}ms${info.error !== undefined ? ` error=${info.error}` : ''}${info.detail !== undefined ? ` detail=${info.detail}` : ''}`)
      : undefined,
  }

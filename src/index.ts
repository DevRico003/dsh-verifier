...
import { OpenAICompatibleBackend, UnconfiguredBackend, placeholderHost, type VerifierBackend } from './core/backend.js'
...
export function apply(ctx: Context, entry: Config): void {
  ...
  if (entry.tools) installTools(ctx, { config, backend: getBackend, log })

...
**Cost.** The verifier thinks (effort low by default). One `verifier_assess` is three parallel calls, about two to four minutes on a long turn. `verifier_select` over three candidates is about thirty calls. Place gates where they pay: one per completed multi-file node, one per merge, one before the final answer. For N such nodes expect at least N + 1 calls; a long build with two calls means the node gates were skipped.

...
export function parseViewport(spec) {
    ...
    if (match === null)
        throw new Error(`ui_snapshot: viewport "${spec}" must look like 1440x900`);
    return { width: Number(match[1]), height: Number(match[2]) };
}
export function snapshotRoot(configured) {
    if (configured !== '')
...
async function loadPlaywright() {
    try {
        ...
        throw new Error(`ui_snapshot: playwright-core is not installed next to dsh-verifier (${String(error)}); run pnpm install in the plugin checkout`);
    }
}
async function launch(pw, channels, headless) {
    const errors = [];
...
export async function takeSnapshots(options) {
        ...
        for (const colorScheme of options.colorSchemes) {
            for (const viewport of options.viewports) {
                if (options.signal?.aborted === true)
                    throw new Error('ui_snapshot: aborted');
                ...
                page.on('console', message => {
                    if (message.type() === 'error')
                        consoleErrors.push(`[${tag}] ${message.text()}`);
                ...
                try {
                    ...
                    if (options.waitForSelector !== undefined)
                        await page.waitForSelector(options.waitForSelector, { timeout: options.navigationTimeoutMs });
...
export function installSnapshotTool(ctx, config) {
    ctx.tools.register(defineTool({
        name: 'ui_snapshot',
        description: 'Headless screenshots of one URL across several viewports and colour schemes in a single call, written as PNG files, '
        ...
        async execute(args, exec) {
            const { url, viewports, colorSchemes, fullPage, waitForSelector, settleMs, label } = args;
            if (!/^https?:\/\//.test(url))
                throw new Error('ui_snapshot: url must start with http:// or https://');
            const snapshot = config().snapshot;
            const schemes = (colorSchemes ?? ['light', 'dark']).map(scheme => {
                if (scheme !== 'light' && scheme !== 'dark')
                    throw new Error(`ui_snapshot: colour scheme "${scheme}" must be light or dark`);
                return scheme;
            });
            ...
            return { ...result, shots: result.shots.map(shot => ({ ...shot })) };
        },
    }));
}
//# sourceMappingURL=ui-snapshot.js.map
...
export function resolveCriteria(name) {
    ...
    if (set === undefined)
        throw new Error(`dsh-verifier-gate: unknown criteria set "${name}" (known: ${CRITERIA_SET_NAMES.join(', ')})`);
    return set;
}

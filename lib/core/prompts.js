...
const GENERAL = {
    groundTruthNote: '**Do NOT trust the agent\'s self-assessment or claims of success.** Judge only what the answer actually contains and what the observed tool output actually shows.',
    criteria: [
        {
            id: 'correctness',
            name: 'Correctness',
            description: 'Compare the agent\'s final answer against what the task actually asked for. Check computations, claims, code and reasoning steps for errors. Score HIGH only when the content is right; score LOW for wrong facts, broken logic, invented details, or an answer to a different question. Ignore style, length and formatting.',
        },
        {
            id: 'completeness',
            name: 'Completeness',
            description: 'Does the response address EVERY explicit requirement, constraint and format the task demands (language, structure, counts, file names, output shape)? Score HIGH when all requirements are met; LOW when parts are missing, silently narrowed, or constraints were violated. Ignore whether the content itself is correct; judge coverage and adherence only.',
        },
        {
            id: 'grounding',
            name: 'Grounding',
 ...
 */
export function buildProgressPrompt(problem, trajectory, nSteps) {
    return [

 ...
 */
import { ASSESS_TAG, buildAssessmentPrompt, buildPairwisePrompt, PAIRWISE_TAG_A, PAIRWISE_TAG_B } from './prompts.js';
...
function fallbackDetail(completion) {
...
export async function select(problem, candidates, options) {
    if (candidates.length === 0)
        throw new Error('dsh-verifier-gate: select needs at least one candidate');
    ...
    const samples = await runWarm(jobs.map(({ criterion, repeat }) => ({ key: 'assess', run: () => run(async () => {
            ...
            for (let attempt = 0; attempt <= retries; attempt++) {
                ...
                try {
                    ...
                    if (options.onError === 'raise')
                        throw error;
                    last = { score: NEUTRAL_SCORE, analysis: `verifier call failed: ${describeError(error)}`, source: 'error' };
                }
                if (isScored(last.source))
                    break;
            }

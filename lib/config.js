...
export function validateConfig(config) {
    ...
    if (gate.threshold < 0 || gate.threshold > 1)
        throw new Error(`dsh-verifier-gate: gate.threshold must be within [0, 1], got ${gate.threshold}`);
    if (!Number.isInteger(gate.maxRounds) || gate.maxRounds < 0)
        throw new Error(`dsh-verifier-gate: gate.maxRounds must be an integer >= 0, got ${gate.maxRounds}`);
    if (!Number.isInteger(gate.evaluations) || gate.evaluations < 1)
        throw new Error(`dsh-verifier-gate: gate.evaluations must be an integer >= 1`);
    if (gate.criteria !== 'auto' && !CRITERIA_SET_NAMES.includes(gate.criteria))
        throw new Error(`dsh-verifier-gate: gate.criteria must be auto or one of ${CRITERIA_SET_NAMES.join(', ')}, got "${gate.criteria}"`);
    if (!CRITERIA_SET_NAMES.includes(select.criteria))
        throw new Error(`dsh-verifier-gate: select.criteria must be one of ${CRITERIA_SET_NAMES.join(', ')}, got "${select.criteria}"`);
    if (!Number.isInteger(select.evaluations) || select.evaluations < 1)
        throw new Error(`dsh-verifier-gate: select.evaluations must be an integer >= 1`);
    if (!Number.isInteger(select.pivots) || select.pivots < 1)
        throw new Error(`dsh-verifier-gate: select.pivots must be an integer >= 1`);
    if (!Number.isInteger(backend.topLogprobs) || backend.topLogprobs < 1 || backend.topLogprobs > 20)
        throw new Error(`dsh-verifier-gate: backend.topLogprobs must be an integer in [1, 20]`);
    if (!Number.isInteger(backend.concurrency) || backend.concurrency < 1)
        throw new Error(`dsh-verifier-gate: backend.concurrency must be an integer >= 1`);
    if (!/^https?:\/\//.test(backend.baseURL))
        throw new Error(`dsh-verifier-gate: backend.baseURL must be an http(s) URL, got "${backend.baseURL}"`);
}

...
export function validateConfig(config) {
    ...
    if (!Number.isInteger(config.toolEvaluations) || config.toolEvaluations < 1)
        throw new Error(`dsh-verifier-gate: toolEvaluations must be an integer >= 1`);
    ...
    if (config.checkpoint.threshold < 0 || config.checkpoint.threshold > 1)
        throw new Error(`dsh-verifier-gate: checkpoint.threshold must be within [0, 1]`);
    if (!Number.isInteger(config.checkpoint.everySteps) || config.checkpoint.everySteps < 1)
        throw new Error(`dsh-verifier-gate: checkpoint.everySteps must be an integer >= 1`);

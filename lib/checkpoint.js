...
async function runCheckpoint(agent, turn, step, state, config, deps, signal) {
    ...
    const base = {
        ...
        onCall: config.verbose
            ? (info) => deps.log.info(`dsh-verifier-gate: checkpoint ${info.kind} ${info.criterion}#${info.repeat} ${info.source} ${info.durationMs}ms${info.error !== undefined ? ` error=${info.error}` : ''}`)
            : undefined,
    };

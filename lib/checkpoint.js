...
export function checkpointTrigger(score, previous, threshold, drop) {
    if (score < threshold)
        return `progress ${score.toFixed(2)} below ${threshold.toFixed(2)}`;
    if (previous !== undefined && previous - score >= drop)
...
export function installCheckpoint(ctx, deps) {
    ...
    ctx.on('agent/pre-step', async ({ agent, turn, step, signal }, next) => {
        ...
        if (config.gate.skipSubagents) {
            ...
            if (meta?.parentSession !== undefined && meta.parentSession !== null)
                return decision;
        }

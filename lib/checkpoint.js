...
import { assess, progress } from './core/verifier.js';
 ...
 */
export function checkpointTrigger(score, previous, threshold, drop, minRise) {
    if (previous === undefined)
        return undefined;
    if (previous - score >= drop)
        return `progress fell from ${previous.toFixed(2)} to ${score.toFixed(2)}`;
    if (score < threshold && score < previous + minRise)
        return `progress ${score.toFixed(2)} stalled below ${threshold.toFixed(2)} (previous ${previous.toFixed(2)})`;
    return undefined;
}
/** The checkpoint message: the measured progress, then the assessment's findings. */
export function renderCheckpoint(step, progressScore, reason, result, threshold, maxChars) {
...
export function renderDebtNudge(edits, repeat = 0) {
...
export function installCheckpoint(ctx, deps) {
    const states = new WeakMap();
    ctx.on('agent/pre-step', async ({ agent, turn, step, signal }, next) => {
        const decision = await next();
        const config = deps.config();
        const checkpoint = config.checkpoint;
        if (decision.kind === 'reject' || signal.aborted || !config.enabled || !checkpoint.enabled)
            return decision;
        if (config.gate.skipSubagents && isChildAgent(agent))
            return decision;
        const previous = states.get(agent);
        const state = previous !== undefined && previous.turn === turn
            ? previous
        ...
        if (step < checkpoint.minSteps || step - state.lastCheckpointStep < checkpoint.everySteps)
            return decision;
        if (state.inFlight || state.steers >= checkpoint.maxSteers)
            return decision;
...
async function runCheckpoint(agent, turn, step, state, config, deps, signal) {
    const checkpoint = config.checkpoint;
    const trajectory = buildTrajectory(agent.session.events, turn, config.trajectory);
    if (trajectory.task.trim() === '' || trajectory.steps === 0)
        return;
    ...
    const base = {
        ...
        onCall: config.verbose
            ? (info) => deps.log.info(`dsh-verifier-gate: checkpoint ${info.kind} ${info.criterion}#${info.repeat} ${info.source} ${info.durationMs}ms${info.error !== undefined ? ` error=${info.error}` : ''}${info.detail !== undefined ? ` detail=${info.detail}` : ''}`)
            : undefined,
    };
    ...
    if (reason !== undefined && reason.includes('stalled')) {
        state.stalls++;
        if (state.stalls < checkpoint.stallReadings)
            reason = undefined;
    }
...
async function assessForSteer(agent, turn, pending, config, deps, signal) {
    const checkpoint = config.checkpoint;
    const trajectory = buildTrajectory(agent.session.events, turn, config.trajectory);
